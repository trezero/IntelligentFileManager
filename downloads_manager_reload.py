#!/usr/bin/env python3
"""
Downloads Manager - Improved with better file handling and UI reload support
"""
...
class DownloadsManagerHandler(SimpleHTTPRequestHandler):
    ...
    # Removed duplicate OPTIONS handler
                    ...
                    try:
                        ...
                        if IS_WSL:
                            if file_path.startswith('C:'):
                                src_path = windows_path_to_wsl(file_path)
                            else:
                                src_path = file_path
                                
                            if destination.startswith('C:'):
                                dest_dir = windows_path_to_wsl(destination)
                        ...
                        if os.path.exists(src_path):
                            ...
                            if IS_WSL and file_path.startswith('C:'):
                                ps_script = f"Move-Item -Path '{file_path}' -Destination '{destination}' -Force"
